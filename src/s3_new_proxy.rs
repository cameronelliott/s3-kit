...
impl Proxy {
    ...
    pub fn new() -> Result<Self> {
        Ok(Self {
            target: BTreeMap::<String, Box<dyn S3 + 'static>>::new(),
        })
    }
}
...
impl S3 for Proxy {
    ...
    async fn get_object(
        ...
        //
        let key = input.key;
        let binding = self.target.read().await;
    ...
    async fn put_object(
        ...
        let mut objects = self.target.write().await;
    ...
    async fn delete_object(
        ...
        if self.target.write().await.remove(&key).is_none() {
            return Err(s3_error!(NoSuchKey));
        }
